...
- List: `GET /center/{centerId}/active-students?page&limit&search`. Live row includes `halqa: { id, name } | null` (Nest PR #4 / `GetActiveStudentDto`; coerce string id). List cards link to `/admin/halaqat/:id` or muted «غير معيّن». No extra roster / by-term invert. Manual add: `POST /admin/student-requests/manual-create` (no `centerId`; JWT-scoped). Then re-list active-students — manual users do **not** appear in the requests queue.
